 ...
 **********************************************************************}
{_$MODE OBJFPC}
unit FPCTypes4Delphi;

  interface
{_$modeswitch advancedrecords}
{_$modeswitch class}
{$LEGACYIFEND OFF}
{$if defined(win32) or defined(win64) or defined(wince)}
    uses
       Windows,
       BasicFPCTypes4Delphi;
{$elseif defined(win16)}
    uses
       WinTypes;
{$else}
uses BasicFPCTypes4Delphi;
{$endif}
...
type
  TEndian =   (Little,Big); // Objpas.TEndian;
...
{$if defined(FPC) and (defined(win32) or defined(win64) or defined(wince))}
...
{$endif FPC_REQUIRES_PROPER_ALIGNMENT}
  record
       x,y : Single;
       public
          function Add(const apt: TPoint): TPointF; overload;
          function Add(const apt: TPointF): TPointF; overload;
          function Distance(const apt : TPointF) : Single;
          function DotProduct(const apt : TPointF) : Single;
          function IsZero : Boolean;
          function Subtract(const apt : TPointF): TPointF; overload;
          function Subtract(const apt : TPoint): TPointF; overload;
          procedure SetLocation(const apt :TPointF); overload;
          ...
          class operator Equal (const apt1, apt2 : TPointF) : Boolean;
          class operator NotEqual (const apt1, apt2 : TPointF): Boolean;
          class operator Add (const apt1, apt2 : TPointF): TPointF;
          class operator Subtract (const apt1, apt2 : TPointF): TPointF;
          class operator Dec (const apt1 : TPointF): TPointF;
          class operator Multiply (const apt1, apt2: TPointF): Single; // scalar product
          class operator Multiply (const apt1: TPointF; afactor: single): TPointF;
          class operator Multiply (afactor: single; const apt1: TPointF): TPointF;
       end;
  { TRectF }
...
type
{$if (not defined(FPC)) or ((not defined(win32)) and (not defined(win64)) and (not defined(wince)))}
  PCLSID = PGUID;
  TCLSID = TGUID;
{$endif}
...
function Size(AWidth, AHeight: Integer): TSize; inline; overload;
function Size(const ARect: TRect): TSize; overload;

{$ifndef VER3_0}
type
  TBitConverter = class
     class procedure UnsafeFrom<T>(const ASrcValue: T; var ADestination: Array of Byte; AOffset: Integer = 0); static; {inline;}
     class procedure From<T>(const ASrcValue: T; var ADestination: Array of Byte; AOffset: Integer = 0); static;
     class function UnsafeInTo<T>(const ASource: Array of Byte; AOffset: Integer = 0): T; static; {inline;}
     class function InTo<T>(const ASource: Array of Byte; AOffset: Integer = 0): T; static;
  end;
{$endif}
...
class operator TPointF.Equal (const apt1, apt2 : TPointF) : Boolean;
begin
  result:=SameValue(apt1.x,apt2.x) and SameValue(apt1.y,apt2.y);
end;

class operator TPointF.NotEqual (const apt1, apt2 : TPointF): Boolean;
begin
  result:=NOT (SameValue(apt1.x,apt2.x) and Samevalue(apt1.y,apt2.y));
end;

class operator TPointF.Multiply (const apt1, apt2: TPointF): Single;
begin
  result:=apt1.x*apt2.x + apt1.y*apt2.y;
end;

class operator TPointF.Multiply (afactor: single; const apt1: TPointF): TPointF;
begin
  result:=apt1.Scale(afactor);
end;

class operator TPointF.Multiply (const apt1: TPointF; afactor: single): TPointF;
begin
  result:=apt1.Scale(afactor);
end;

class operator TPointF.Add (const apt1, apt2 : TPointF): TPointF;
begin
  result.x:=apt1.x+apt2.x;
  result.y:=apt1.y+apt2.y;
end;

class operator TPointF.Subtract (const apt1, apt2 : TPointF): TPointF;
begin
  result.x:=apt1.x-apt2.x;
  result.y:=apt1.y-apt2.y;
end;

class operator TPointF.Dec (const apt1: TPointF): TPointF;
begin
  Result.x:=-apt1.x;
  Result.y:=-apt1.y;
end;
...
{$ifndef VER3_0}
class procedure TBitConverter.UnsafeFrom<T>(const ASrcValue: T; var ADestination: Array of Byte; AOffset: Integer = 0);
begin
  move(ASrcValue, ADestination[AOffset], SizeOf(T));
end;

class procedure TBitConverter.From<T>(const ASrcValue: T; var ADestination: Array of Byte; AOffset: Integer = 0);
begin
  if AOffset < 0 then
  ...
  TBitConverter.UnsafeFrom<T>(ASrcValue, ADestination, AOffset);
end;

class function TBitConverter.UnsafeInTo<T>(const ASource: Array of Byte; AOffset: Integer = 0): T;
begin
  move(ASource[AOffset], Result, SizeOf(T));
end;

class function TBitConverter.InTo<T>(const ASource: Array of Byte; AOffset: Integer = 0): T;
begin
  if AOffset < 0 then
    System.Error(reRangeError);
  ...
  Result := TBitConverter.UnsafeInTo<T>(ASource, AOffset);
end;
{$endif}
