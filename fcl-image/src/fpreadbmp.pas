...
uses FPImage, classes, sysutils, BMPcomn
     {$IFNDEF FPC}
     , FPCTypes4Delphi, BasicFPCTypes4Delphi
     {$ENDIF}
     ;
...
begin
  ...
  begin
    ...
    for i := 0 to High(ColInfo) do
      ...
      {$ELSE}
      PFPColorArray(FPalette)[i] := RGBAToFPColor(ColInfo[i]);
      {$ENDIF}
    end
  ...
  Try
    ...
    begin
      ...
      for i:=0 to pallen-1 do
        Img.Palette.Color[i]:=PFPColorArray(FPalette)[i];
    end;
...
begin
  Case BFI.BitCount of
   ...
   16 :
      for Column:=0 to img.Width-1 do
        img.colors[Column,Row]:=ExpandColor({$IFDEF FPC}PWord {$ELSE}PWordArray{$ENDIF}(LineBuf)[Column]);
   24 :
      for Column:=0 to img.Width-1 do
        img.colors[Column,Row]:=RGBToFPColor({$IFDEF FPC}PColorRGB{$ELSE}PColorRGBArray{$ENDIF}(LineBuf)[Column]);
   32 :
      for Column:=0 to img.Width-1 do
        if BFI.Compression=BI_BITFIELDS then
          img.colors[Column,Row]:=ExpandColor({$IFDEF FPC}PLongWord{$ELSE}PLongWordArray{$ENDIF}(LineBuf)[Column])
        else
          img.colors[Column,Row]:=RGBAToFPColor({$IFDEF FPC}PColorRGBA{$ELSE}PColorRGBAArray{$ENDIF}(LineBuf)[Column]);
    end;
