...
type
 TFPColorHelper =  record helper for TFPColor
   ...
   class operator NotEqual (const c,d:TFPColor) : boolean; inline;
   class operator BitwiseOr (const c,d:TFPColor) : TFPColor;
   class operator BitwiseAnd (const c,d:TFPColor) : TFPColor;
   class operator BitwiseXor (const c,d:TFPColor) : TFPColor;
   class function CompareColors(const Color1, Color2: TFPColor): integer; static;
   end;
{$ELSE}
...
Type
  ...
  TFPCompactImgGrayAlpha16BitValueArray = array [0..(maxint - 1) div SizeOf(TFPCompactImgGrayAlpha16BitValue) - 1] of TFPCompactImgGrayAlpha16BitValue;
  PFPCompactImgGrayAlpha16BitValueArray = ^TFPCompactImgGrayAlpha16BitValueArray;
  ...
  TFPCompactImgGrayAlpha8BitValueArray = array [0..0] of TFPCompactImgGrayAlpha8BitValue;
  ...
  TFPCompactImgRGBA8BitValueArray = Array[0..(maxint - 1) div SizeOf(TFPCompactImgRGBA8BitValue) - 1] of TFPCompactImgRGBA8BitValue;
  PFPCompactImgRGBA8BitValueArray = ^TFPCompactImgRGBA8BitValueArray;
  ...
  TFPCompactImgRGB8BitValueArray = Array[0..(maxint - 1) div SizeOf(TFPCompactImgRGB8BitValue) -1] of TFPCompactImgRGB8BitValue;
  PFPCompactImgRGB8BitValueArray = ^TFPCompactImgRGB8BitValueArray;
  ...
  TFPCompactImgRGB16BitValueArray = array [0..(maxint - 1) div SizeOf(TFPCompactImgRGB16BitValue) - 1] of TFPCompactImgRGB16BitValue;
  PFPCompactImgRGB16BitValueArray = ^TFPCompactImgRGB16BitValueArray;
...
procedure FPImgError (Fmt:TErrorTextIndices; data : array of const);  {$IFNDEF FPC} overload; {$ENDIF}
begin
  raise FPImageException.CreateFmt (ErrorText[Fmt],data);
end;

procedure FPImgError (Fmt:TErrorTextIndices); {$IFNDEF FPC} overload; {$ENDIF}
begin
...
{$IFNDEF FPC}
class operator TFPColorHelper.Equal (const c,d:TFPColor) : boolean;
{$ELSE}
operator = (const c,d:TFPColor) : boolean;
{$ENDIF}
begin
...
{$IFNDEF FPC}
class operator TFPColorHelper.NotEqual (const c,d:TFPColor) : boolean;
begin
  result:= not (c = d);
end;
{$ENDIF}
...
{$IFNDEF FPC}
class operator TFPColorHelper.BitwiseOr (const c,d:TFPColor) : TFPColor;
{$ELSE}
operator or (const c,d:TFPColor) : TFPColor;
{$ENDIF}
begin
...
{$IFNDEF FPC}
class operator TFPColorHelper.BitwiseAnd (const c,d:TFPColor): TFPColor;
{$ELSE}
operator and (const c,d:TFPColor) : TFPColor;
{$ENDIF}
begin
...
{$IFNDEF FPC}
class operator TFPColorHelper.BitwiseXor (const c,d:TFPColor) : TFPColor;
{$ELSE}
operator xor (const c,d:TFPColor) : TFPColor;
{$ENDIF}
begin
...
begin
   Result := True;
   {$IFNDEF FPC}
   colorname:= LowerCase(s);
   if colorname = 'white'   then AName := hcnWhite else
   if colorname = 'silver'  then AName := hcnSilver else
   if colorname = 'gray'    then AName := hcnGray else
   if colorname = 'black'   then AName := hcnBlack else
   if colorname = 'red'     then AName := hcnRed else
   if colorname = 'maroon'  then AName := hcnMaroon else
   if colorname = 'yellow'  then AName := hcnYellow else
   if colorname = 'olive'   then AName := hcnOlive else
   if colorname = 'lime'    then AName := hcnLime else
   if colorname = 'green'   then AName := hcnGreen else
   if colorname = 'aqua'    then AName := hcnAqua else
   if colorname = 'teal'    then AName := hcnTeal else
   if colorname = 'blue'    then AName := hcnBlue else
   if colorname = 'navy'    then AName := hcnNavy else
   if colorname = 'fuchsia' then AName := hcnFuchsia else
   if colorname = 'purple'  then AName := hcnPurple
   {$ELSE}
   case LowerCase(S) of
