...
uses
  SysUtils,Classes, FPImage, FPImgCmn, PNGComn,
  {$IFDEF FPC}
  ZStream
  {$ELSE}
  IdZLib, FPCTypes4Delphi
  {$ENDIF}
  ;
...
Type
  ...
  TFPReaderPNG = class (TFPCustomImageReader)
    ...
    protected
      ...
      {$ELSE}
      procedure DecideSetPixel(var aProc: TSetPixelProc); virtual;
      {$ENDIF}
...
{$ELSE}
procedure TFPReaderPNG.DecideSetPixel(var aProc: TSetPixelProc);
begin
  if Pltte then
    if TheImage.UsePalette then
      aProc := SetPalettePixel
    else
      aProc := SetPalColPixel
  else
    if UseTransparent then
      aProc := SetColorTrPixel
    else
      aProc := SetColorPixel;
end;
{$ENDIF}
