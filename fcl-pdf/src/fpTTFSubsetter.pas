...
type
  ...
  private
    ...
    function    ToUInt32(const AHigh, ALow: UInt32): UInt32; {$IFNDEF FPC} overload; {$ENDIF}
    function    ToUInt32(const ABytes: AnsiString): UInt32; {$IFNDEF FPC} overload; {$ENDIF}
  ...
  public
    constructor Create(const AFont: TTFFileInfo; const AGlyphIDList: TTextMappingList); {$IFNDEF FPC} overload; {$ENDIF}
    constructor Create(const AFont: TTFFileInfo); {$IFNDEF FPC} overload; {$ENDIF}
    ...
    try
      ...
      begin
        ...
        for n := 0 to numLocations-1 do
          ...
          {$ELSE}
          begin
          ms.ReadData(readAWord);
          FGlyphLocations[n]:= BEtoN(readAWord) * 2;
          end;
          {$ENDIF}
      end
      else
      begin
        ...
        for n := 0 to numLocations-1 do
            ...
            {$ELSE}
            begin
            ms.ReadData(FGlyphLocations[n]);
            FGlyphLocations[n]:= BEtoN(FGlyphLocations[n]);
            end;
            {$ENDIF}
      end;
    finally
...
begin
  ...
  {$ELSE}
  AStream.WriteData(NtoBE(AValue));
  {$ENDIF}
end;
...
begin
  ...
  newOffsets[FGlyphIDs.Count] := lOffset;
end;
