...
type
  ...
  public
    Constructor Create(Const ADocument : TPDFDocument); virtual;
  end;
  ...
  public
    constructor Create(Const ADocument : TPDFDocument; const AValue: Boolean);{$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  end;
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    class function Command(APos: TPDFCoord): String; {$IFNDEF FPC} overload; {$ENDIF}
    class function Command(AX,AY: TPDFFloat): String; {$IFNDEF FPC} overload; {$ENDIF}
    constructor Create(Const ADocument : TPDFDocument; const AX,AY : TPDFFloat); {$IFDEF FPC} overload; {$ELSE} reintroduce; overload; {$ENDIF}
    constructor Create(Const ADocument : TPDFDocument; const APos : TPDFCoord); {$IFDEF FPC} overload; {$ELSE} reintroduce; overload; {$ENDIF}
  end;
  ...
  protected
    ...
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const AValue: integer); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const AValue: string; const AMustEscape: boolean = True); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const AValue: AnsiString); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const AValue: UnicodeString; const AFontIndex : Integer); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const AValue: String); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const AValue: UTF8String; const AFontIndex: integer); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  private
    FArray: {$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF};
  protected
    procedure Write(const AStream: TStream); override;
  ...
  private
    ...
    FItems: {$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF};
  protected
    procedure Write(const AStream: TStream); override;
    procedure AddItem(const AValue: TPDFObject);
  public
    constructor Create(Const ADocument : TPDFDocument; OwnsObjects : Boolean = True); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
    destructor Destroy; override;
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; AStream: TStream); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
    destructor Destroy; override;
  end;
  ...
  public
    constructor Create(const ADocument: TPDFDocument;const APage: TPDFPage; const AFont: integer; const ASize: string); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  private
    ...
    function    GetTextHeight: single; overload;
  protected
    procedure   Write(const AStream: TStream); override;
  public
    constructor Create(const ADocument: TPDFDocument; const AX, AY: TPDFFloat; const AText: AnsiString; const AFont: TPDFEmbeddedFont; const ADegrees: single; const AUnderline: boolean; const AStrikethrough: boolean); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
    destructor  Destroy; override;
    class function GetTextHeight(const aFontName: string; fontPointSize: integer): single; overload;
  ...
  protected
    procedure   Write(const AStream: TStream); override;
  public
    constructor Create(const ADocument: TPDFDocument; const AX, AY: TPDFFloat; const AText: UTF8String; const AFont: TPDFEmbeddedFont; const ADegrees: single; const AUnderline: boolean; const AStrikethrough: boolean); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
    destructor  Destroy; override;
  ...
  protected
    procedure   Write(const AStream: TStream); override;
  public
    constructor Create(const ADocument: TPDFDocument; const AX, AY: TPDFFloat; const AText: UnicodeString; const AFont: TPDFEmbeddedFont; const ADegrees: single; const AUnderline: boolean; const AStrikethrough: boolean); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
    destructor  Destroy; override;
  ...
  public
    ...
    constructor Create(Const ADocument : TPDFDocument; const AWidth, X1,Y1, X2,Y2: TPDFFloat; const AStroke: Boolean = True); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  end;
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(const ADocument: TPDFDocument; const APosX, APosY, AWidth, AHeight, ALineWidth: TPDFFloat; const AFill, AStroke: Boolean);{$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  end;
  ...
  protected
    procedure   Write(const AStream: TStream); override;
  public
    constructor Create(const ADocument: TPDFDocument; const APosX, APosY, AWidth, AHeight, ARadius, ALineWidth: TPDFFloat; const AFill, AStroke: Boolean);{$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  end;
  ...
  public
    ...
    constructor Create(Const ADocument : TPDFDocument; const xCtrl1, yCtrl1, xCtrl2, yCtrl2, xTo, yTo, AWidth: TPDFFloat; AStroke: Boolean = True);{$IFNDEF FPC} reintroduce; {$ENDIF} overload;
    constructor Create(Const ADocument : TPDFDocument; const ACtrl1, ACtrl2, ATo3: TPDFCoord; AWidth: TPDFFloat; AStroke: Boolean = True);{$IFNDEF FPC} reintroduce; {$ENDIF} overload;
  end;
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const X2,Y2,X3,Y3,AWidth : TPDFFloat;AStroke: Boolean = True);{$IFNDEF FPC} reintroduce; {$ENDIF} overload;
    constructor Create(Const ADocument : TPDFDocument; const AP2,AP3 : TPDFCoord; AWidth: TPDFFloat;AStroke: Boolean = True);{$IFNDEF FPC} reintroduce; {$ENDIF} overload;
  end;
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const X1,Y1,X3,Y3,AWidth : TPDFFloat;AStroke: Boolean = True);{$IFNDEF FPC} reintroduce; {$ENDIF} overload;
    constructor Create(Const ADocument : TPDFDocument; const AP1,AP3 : TPDFCoord; AWidth: TPDFFloat;AStroke: Boolean = True);{$IFNDEF FPC} reintroduce; {$ENDIF} overload;
  end;
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const APosX, APosY, AWidth, AHeight, ALineWidth: TPDFFloat; const AFill : Boolean = True; AStroke: Boolean = True);{$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  end;
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const APoints: TPDFCoordArray; AClose : Boolean; AFill : Boolean = True); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  end;
  ...
  protected
    procedure Write(const AStream: TStream); override;
  public
    constructor Create(Const ADocument : TPDFDocument; const ALeft, ABottom, AWidth, AHeight: TPDFFloat; ANumber: integer); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  end;
  ...
  protected
    procedure Write(const AStream: TStream);override;
  public
    constructor Create(Const ADocument : TPDFDocument; AStyle: TPDFPenStyle; APhase: integer; ALineWidth: TPDFFloat); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  end;
  ...
  public
    ...
    constructor Create(Const ADocument : TPDFDocument; const AStroke: Boolean; AColor: TARGBColor); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  protected
    procedure Write(const AStream: TStream);override;
  public
    constructor Create(Const ADocument : TPDFDocument; const AKey: string; const AValue: TPDFObject); {$IFNDEF FPC} reintroduce; {$ENDIF}
    destructor Destroy; override;
  ...
  private
    FElements: {$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF}; // list of TPDFDictionaryItem
  ...
  protected
    ...
    procedure AddString(const AKey, AString : String); {$IFNDEF FPC} overload; {$ENDIF}
//    procedure AddString(const AKey:string;const AString : UnicodeString); {$IFNDEF FPC} overload; {$ENDIF}
  ...
  private
    FTitle: String;
    FPages : {$IFDEF FPC} TFPList {$ELSE} TList {$ENDIF}; // not owned
    function GetP(AIndex : Integer): TPDFPage; {$IFNDEF FPC} overload; {$ENDIF}
    function GetP: Integer; {$IFNDEF FPC} overload; {$ENDIF}
  Public
    Destructor Destroy; override;
  ...
  protected
    procedure Write(const AStream: TStream);override;
  public
    constructor Create(const ADocument: TPDFDocument; const AEmbeddedFontNum: integer); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  private
    FList: {$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF};
    FPageClass: TPDFPageClass;
    function    GetP(AIndex: Integer): TPDFPage;
    function    GetPageCount: integer;
  public
    constructor Create(const ADocument: TPDFDocument); override; {$IFDEF FPC} overload; {$ENDIF}
    destructor  Destroy; override;
  ...
  private
    ...
    FBorder: boolean;
  public
    constructor Create(const ADocument: TPDFDocument); {$IFNDEF FPC} overload; {$ENDIF} override; {$IFDEF FPC} overload; {$ENDIF}
    constructor Create(const ADocument: TPDFDocument; const ALeft, ABottom, AWidth, AHeight: TPDFFloat; const AURI: String; const ABorder: Boolean = false); {$IFNDEF FPC}reintroduce; {$ENDIF} overload;
  end;
  ...
  Public
    ...
    {$IFNDEF FPC}
    function Owner: TPDFDocument; reintroduce; inline;
    {$ELSE}
  ...
  protected
    FFontNum: integer;
  public
    constructor Create(const ADocument: TPDFDocument; const AFontNum: integer); {$IFDEF FPC} overload; {$ELSE} reintroduce; {$ENDIF}
  ...
  private
    ...
    FZoomValue: string;
    FGlobalXRefs: {$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF}; // list of TPDFXRef
    FUnitOfMeasure: TPDFUnitOfMeasure;
    class function GetStdFontCharWidthsArray(const AFontName: string): TPDFFontWidthArray;
  ...
  public
    ...
    destructor Destroy; override;
    class function TextWidthOfStandardFont(const aText, aStdFontName: string; aPointSize: integer): Single;
    procedure StartDocument;
    procedure Reset;
    procedure SaveToStream(const AStream: TStream); virtual;
    Procedure SaveToFile(Const AFileName : String);
    class function  IsStandardPDFFont(AFontName: string): boolean;
...
uses
  {$IFNDEF FPC}
  BasicFPCTypes4Delphi, FPCTypes4Delphi,
  DateUtils,
  {$IFDEF ANDROID}
  System.IOUtils,
  {$ENDIF}
...
begin
  ...
  c := TCompressionStream.Create(ACompressLevel, ATo, {$IFNDEF FPC} not {$ENDIF} ASkipHeader);
  try
    AFrom.Position := 0;
...
{$IFDEF NOHEADERWORKADOUND}
  ...
  AFrom.Position:=0;
{$ENDIF}
  D:=TDecompressionStream.Create(AFrom {$IFDEF FPC} , False {$ENDIF});
  try
    repeat
...
const
    ...
    {$ELSE}
    NBSP: UnicodeString = WideChar($FEFF);
    {$ENDIF}
begin
  WriteString('<?xpacket begin="'+ {$IFDEF FPC} UnicodeCharToString(@NBSP) {$ELSE} NBSP {$ENDIF} +'" id="W5M0MpCehiHzreSzNTczkc9d"?>'+CRLF, AStream);
...
begin
  if (FList=Nil) then
    FList:={$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF}.Create;
...
begin
  if Not Assigned(FPages) then
    FPages:={$IFDEF FPC} TFPList {$ELSE} TList {$ENDIF}.Create;
  FPages.Add(APage);
end;
    ...
    for Y:=0 to FHeight-1 do
      for X:=0 to FWidth-1 do
        ...
        if CreateMask then
          ...
          {$ELSE}
          StrMask.WriteData(Byte(C.Alpha shr 8))
          {$ENDIF}
        else
        ...
        {$ELSE}
        Str.WriteData(Byte(C.Red shr 8));
        Str.WriteData(Byte(C.Green shr 8));
        Str.WriteData(Byte(C.Blue shr 8));
        {$ENDIF}
        end;
...
{$IFNDEF FPC}
function TPDFImages.Owner: TPDFDocument;
begin
  result:= FOwner;
end;
{$ENDIF}
...
constructor TPDFString.Create(Const ADocument : TPDFDocument; const AValue: AnsiString);
begin
  inherited Create(ADocument);
  FValue := AValue;
...
begin
  inherited Create(ADocument);
  FArray:={$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF}.Create;
end;
...
begin
  inherited Create(ADocument);
  FItems:={$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF}.Create(OwnsObjects);
end;
...
begin
  result:= TPDFText.GetTextHeight(Document.Fonts[Font.FontIndex].Name, Font.PointSize);
end;

class function TPDFText.GetTextHeight(const aFontName: string; fontPointSize: integer): single;
begin
  Result := 0;
  {$IFDEF FPC}
  case aFontName of
  ...
  {$ELSE}
  if aFontName = 'Courier' then                result := FONT_TIMES_COURIER_CAPHEIGHT
  else
  if aFontName = 'Courier-Bold' then           result := FONT_TIMES_COURIER_CAPHEIGHT
  else
  if aFontName = 'Courier-Oblique' then        result := FONT_TIMES_COURIER_CAPHEIGHT
  else
  if aFontName = 'Courier-BoldOblique' then    result := FONT_TIMES_COURIER_CAPHEIGHT
  else
  if aFontName = 'Helvetica' then              result := FONT_HELVETICA_ARIAL_CAPHEIGHT
  else
  if aFontName = 'Helvetica-Bold' then         result := FONT_HELVETICA_ARIAL_BOLD_CAPHEIGHT
  else
  if aFontName = 'Helvetica-Oblique' then      result := FONT_HELVETICA_ARIAL_ITALIC_CAPHEIGHT
  else
  if aFontName = 'Helvetica-BoldOblique' then  result := FONT_HELVETICA_ARIAL_BOLD_ITALIC_CAPHEIGHT
  else
  if aFontName = 'Times-Roman' then            result := FONT_TIMES_CAPHEIGHT
  else
  if aFontName = 'Times-Bold' then             result := FONT_TIMES_BOLD_CAPHEIGHT
  else
  if aFontName = 'Times-Italic' then           result := FONT_TIMES_ITALIC_CAPHEIGHT
  else
  if aFontName = 'Times-BoldItalic' then       result := FONT_TIMES_BOLD_ITALIC_CAPHEIGHT
  else
  if aFontName = 'Symbol' then                 result := 300
  else
  if aFontName = 'ZapfDingbats' then           result := 300
    else
      raise EPDF.CreateFmt(rsErrUnknownStdFont, [aFontName]);
  {$ENDIF}
  Result := Result * fontPointSize / 1540;
end;
...
begin
  inherited Create(ADocument);
  FElements:={$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF}.Create;
end;
...
class function TPDFDocument.GetStdFontCharWidthsArray(const AFontName: string): TPDFFontWidthArray;
begin
  {$IFDEF FPC}
  case AFontName of
  ...
  {$ELSE}
  if AFontName = 'Courier' then                result := FONT_COURIER_FULL
  else
  if AFontName = 'Courier-Bold' then           result := FONT_COURIER_FULL
  else
  if AFontName = 'Courier-Oblique' then        result := FONT_COURIER_FULL
  else
  if AFontName = 'Courier-BoldOblique' then    result := FONT_COURIER_FULL
  else
  if AFontName = 'Helvetica' then              result := FONT_HELVETICA_ARIAL
  else
  if AFontName = 'Helvetica-Bold' then         result := FONT_HELVETICA_ARIAL_BOLD
  else
  if AFontName = 'Helvetica-Oblique' then      result := FONT_HELVETICA_ARIAL_ITALIC
  else
  if AFontName = 'Helvetica-BoldOblique' then  result := FONT_HELVETICA_ARIAL_BOLD_ITALIC
  else
  if AFontName = 'Times-Roman' then            result := FONT_TIMES
  else
  if AFontName = 'Times-Bold' then             result := FONT_TIMES_BOLD
  else
  if AFontName = 'Times-Italic' then           result := FONT_TIMES_ITALIC
  else
  if AFontName = 'Times-BoldItalic' then       result := FONT_TIMES_BOLD_ITALIC
  else
  if AFontName = 'Symbol' then                 result := FONT_SYMBOL
  else
  if AFontName = 'ZapfDingbats' then           result := FONT_ZAPFDINGBATS
    else
      raise EPDF.CreateFmt(rsErrUnknownStdFont, [AFontName]);
  {$ENDIF}
end;
...
begin
  FGlobalXRefs:={$IFDEF FPC} TFPObjectList {$ELSE} TObjectList {$ENDIF}.Create;
...
class function TPDFDocument.TextWidthOfStandardFont(const aText: string; const aStdFontName: string; aPointSize: integer): Single;
var
  i: integer;
  lWidth: double;
begin
  if not TPDFDocument.IsStandardPDFFont(aStdFontName) then
     raise EPDF.CreateFmt(rsErrUnknownStdFont, [aStdFontName]);
  lWidth := 0;
  for i := 1 to Length(aText) do
      lWidth := lWidth + TPDFDocument.GetStdFontCharWidthsArray(aStdFontName)[Ord(aText[i])];
  Result := lWidth * aPointSize / 1540;
end;
...
begin
  ...
  if (FontDirectory = '') then
     ...
     {$ELSE}
     FontDirectory:= IncludeTrailingPathDelimiter(TPath.GetPublicPath) + 'font' + PathDelim;
     {$ENDIF}
end;
...
class function TPDFDocument.IsStandardPDFFont(AFontName: string): boolean;
begin
...
begin
  result:= '';
  if SHGetSpecialFolderPath(0, path, ID, false) then
...
begin
  ...
  if ExtractFilePath(AFontFile) <> '' then
    ...
    begin
    {$IFDEF MSWINDOWS}
    lFName := SpecialDirectory(CSIDL_FONTS) + AFontFile;
...
initialization
  ...
  {$ELSE}
  PDFFormatSettings:= FormatSettings;
  {$ENDIF}
