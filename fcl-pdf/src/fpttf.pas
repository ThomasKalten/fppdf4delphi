...
uses
  ...
  {$ELSE}
  {$ifdef HasFontsConf}
  Xml.XMLDoc, // Todo: would have to be adapted
  {$ENDIF}
...
//        writeln(lDir);
  ...
  {$IFDEF ANDROID}
  SearchPath.Add('/system/fonts/'); // Samsung verified
  SearchPath.Add(IncludeTrailingPathDelimiter(TPath.GetPublicPath) + 'font' + PathDelim); // Font includes as Asset "assets/font" in App (not "res/font" - resources cannot be handled as files);
  {$ENDIF}
